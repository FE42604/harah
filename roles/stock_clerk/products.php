...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['action'])) {
    if ($_POST['action'] === 'add_product') {
        ...
        $query = "INSERT INTO tbl_products (product_name, category_id, product_selling_price, 
                  product_quantity, product_restock_qty, product_image, product_created_at, employee_id, product_description) 
                  VALUES (?, ?, ?, ?, ?, ?, NOW(), ?, ?)";
        $stmt = $conn->prepare($query);
        $stmt->bind_param("siddiss", $product_name, $category_id, $product_selling_price, 
                         $product_quantity, $product_restock_qty, $product_image, $_SESSION['employee_id'], $product_description);
